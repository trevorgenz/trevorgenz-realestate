...
export default function NavBar() {
  ...
  const bg = scrolled ? "bg-[#0f172a] shadow-lg" : "bg-transparent";
        ...
        <div className="hidden md:flex items-center gap-6">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="text-white/80 hover:text-[#3b82f6] text-sm font-medium transition-colors"
            >
              {l.label}
            </a>
          ))}
          <a
            href="tel:5412797522"
            className="flex items-center gap-2 bg-[#3b82f6] hover:bg-[#f5a06a] text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors"
          >
        ...
        <div className="md:hidden bg-[#0f172a] border-t border-white/10 px-6 py-4 flex flex-col gap-4">
          {links.map((l) => (
            <a
              ...
              className="text-white/80 hover:text-[#3b82f6] text-sm font-medium"
            >
              {l.label}
            </a>
          ))}
          <a href="tel:5412797522" className="text-[#3b82f6] font-semibold text-sm">
            (541) 279-7522
          </a>
        </div>
      )}
